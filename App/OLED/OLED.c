...
static void OLEDDrawPoint(u8 x, int y, u8 t); //在OLED屏指定位置画点
 ...
 *                字节）相加为7。
 *********************************************************************************************************/
static void OLEDDrawPoint(u8 x, int y, u8 t)
{
	u8 pos;		 //存放点所在的页数
	u8 bx;		 //存放点所在的屏幕的行号
	u8 temp = 0; //用来存放画点位置相对于字节的位

	if (x > 127 || y > 63 || y < 0) //如果指定位置超过额定范围
	{
		return; //返回空，函数结束
	}
 ...
 * 注    意：有6种取模方式
 *********************************************************************************************************/
void OLEDShowChar(u8 x, int y, u8 chr, u8 size, u8 mode)
{
...
#define MAX_CHAR_POSY 80  // OLED屏幕纵向的最大范围
void OLEDShowString(u8 x, int y, const u8 *p)
{
...
void OLEDShowStringBySize12(u8 x, u8 y, const u8 *p)
{
	while (*p != '\0') //指针不等于结束符时，循环进入
	{
		if (x > MAX_CHAR_POSX) //如果x超出指定最大范围，x赋值为0
		...
		OLEDShowChar(x, y, *p, 12, 1); //指定位置显示一个字符
...
void OLEDDrawPointForMap(u8 x, u8 y, u8 t)
{
	u8 pos;		 //存放点所在的页数
	u8 bx;		 //存放点所在的屏幕的行号
	u8 temp = 0; //用来存放画点位置相对于字节的位
